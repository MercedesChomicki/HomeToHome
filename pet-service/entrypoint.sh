...
# Esperar a Postgres
echo "Esperando a pet-postgres:5432 ..."
count=0
until nc -z pet-postgres 5432; do
  count=$((count+1))
  if [ $count -ge 30 ]; then
    echo "⛔ Timeout esperando pet-postgres después de $((count*2)) segundos."
    exit 1
  fi
  echo "pet-postgres aún no está disponible. Reintentando en 2s..."
  sleep 2
done
echo "✅ pet-postgres está listo."

# --- Iniciar el microservicio ---
echo "🚀 Todos los servicios están disponibles. Iniciando pet-service..."
exec java -jar pet-service.jar
